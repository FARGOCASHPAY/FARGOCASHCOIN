...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FRGSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FRGSMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FRGSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fargocash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("FRGSMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running FRGSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadFRGSMiner() exception");
    } catch (...) {
        LogPrintf("ThreadFRGSMiner() exception");
    }

    LogPrintf("ThreadFRGSMiner exiting\n");
}
